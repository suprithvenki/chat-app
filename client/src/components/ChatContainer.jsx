...
const ChatContainer = () => {
    ...
    const { getMessages, messages, selectedUser, setSelectedUser, sendMessage } = useContext(ChatContext);
    ...
    return selectedUser ? (
        <div className='h-full overflow-scroll relative backdrop-blur-lg'>
            {/* Header */}
            <div className="flex items-center gap-3 py-3 mx-4 bordr-b border-stone-500">
                ...
                {messages.map((msg, index) => (
                    <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
                        {msg.image ? (
                            ...
                            : (
                                <p className={`p-2 mb-8 break-all bg-violet-500/30 text-white ${msg.senderId === authUser._id ? 'rounded-br-none' : 'rounded-bl-none'}`}>{msg.text}</p>
                            )}
            ...
            <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
                <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
                    ...
                    <input onChange={handleSendImage}
                        type="file" name="" id="image" accept='image/png image/jpg' hidden />
        ...
        <div className='flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-md:hidden'>
            <img src={assets.logo} alt="" className='max-w-60' />
